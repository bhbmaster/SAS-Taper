...
The result is deterministic — same arguments, same cut — so the drawing never moves under a reader who has changed nothing. **`fold.html`** is a second shipped page explaining all of the above: the whole algorithm in one annotated block, then the same thing as seven live stages driven by a slider. The listing wraps on a phone; Wrap / Scroll on the listing picks the other. It reimplements the search a third time, in the page, deliberately — it is an explainer, not the tool, and nothing doses off it. It is self-contained like `index.html` and the layout sweep checks that, plus that its links home still resolve, and that the glossary still names the listing. `test_parity.js` sweeps 13,920 cases across six film sizes and four tolerances, because a search with a tie-break is exactly the kind of code that drifts between two implementations.
...
`fold.html`'s algorithm listing wraps by default below 620 px, with Wrap / Scroll buttons on the listing itself so a reader can pick the other. The sweep checks both sides of that breakpoint, that each button actually flips the listing, that choosing Scroll on a phone keeps the scroller inside the `<pre>` rather than expanding the page, and that an explicit choice survives a reload. It also greps the glossary against the listing, so the names a reader is taught cannot drift from the block they then read. The widths it uses have to straddle the breakpoint, or a regression that only shows on a phone would never be measured.
