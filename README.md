...
|  | **Same fraction** (default) | **Same amount** |
|---|---|---|
...
Same-amount is the shorter road, and the cut never gets fiddly — the same mark with the same razor from the first day to the last, so there is no sliver under a millimetre and no need to move to 2 mg films for precision. The site turns the 2 mg switch off in that mode and says why.

**It is also steeper where a taper is hardest.** Equal milligrams are growing percentages:

```
8 mg, n = 6, same amount:  6.67 → 5.33 → 4.00 → 2.67 → 1.33 → 0
...
0. Pick a cut mode. Same fraction (default) or same amount — see [Two cut modes](#two-cut-modes). Everything below is the same either way; only the size of the next cut differs.
...
5. Next cycle, the leftover size is the new whole strip. In same-fraction mode that means the next cut is smaller; in same-amount mode you cut the identical piece again. Repeat to the target (default 1 mg), or in same-amount mode until there is nothing left to cut.
