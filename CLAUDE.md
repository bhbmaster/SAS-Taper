...
Working notes for anyone changing this repo, human or agent.
...
- **"Save" is the film in front of you. "sliver" and "banked" are the ladder.** `save_mm` / `save_mg` are everything right of the take mark on a fresh film: the sliver *plus* what earlier cycles already removed. `delta_save_mm` is how much more that is than last cycle. `sliver_mg` and `banked_mg` are the method's own accounting and are deliberately smaller. Do not "fix" one to match the other. They answer different questions and both are checked.
- **`delta_save_mm` is `None` in three cases, not zero**: a 2 mg restart, a cycle that drops a whole film from the day, and a day with no cut. Each one makes "extra saved" a comparison against a different thing. The schedule, cards and calendar all show those as a dash.
...
- **`fraction_cut()` is written twice and is a search with a tie-break.** Three terms are load-bearing and none is obvious: a lengthwise cut costs more than a crosswise one (without it a plain half comes out `1×2`), `tol_mg` caps the error rather than adding to the best one (written as `best + tol` they compound and the cut lands outside the stated tolerance), and grid fineness ranks after error rather than inside the difficulty score (without that, 0.32 mg on an 8 mg film prefers `1/16` over `1/24`: same two cuts, 0.17 mg spent on the coarser-looking grid). All three have tests; all three were wrong first.
- **`fold.html`'s difficulty score is itemised, and the itemisation is the score.** `SCORE_ROWS` is the list of terms; `difficulty()` is nothing but their sum, and stage 02's comparison panel prints the same rows. Add a term to one and not the other and the panel's own arithmetic stops adding up, which `test_layout.js` checks, by totting up the printed rows against the printed total at 241 doses. Grid fineness is deliberately not a row: it is a tie-break after error, and putting it in the score is the 1/16-over-1/24 bug.
- **Nothing the fold search considered can be cheaper than what it chose.** The pool is sorted on difficulty before error, so a pool member with a lower score than the winner is a contradiction, not a near-miss. The comparison panel says so in as many words, and the sweep fails if that sentence ever renders. A closer fold that takes more strokes losing is *normal*: being closer than the winner puts it inside the cap by construction, so it was always in the running and lost on difficulty alone. A closer fold with the same strokes losing is a bug.
- **A fraction cut always states its error in milligrams.** An approximation whose size the reader cannot see is worse than no approximation. Do not add a drawing that shows the fold without the number.
