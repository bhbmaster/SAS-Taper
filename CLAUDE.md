...
- **`fold.html`'s difficulty score is itemised, and the itemisation is the score.** `SCORE_ROWS` is the list of terms; `difficulty()` is nothing but their sum, and stage 02's comparison panel prints the same rows. Add a term to one and not the other and the panel's own arithmetic stops adding up, which `test_layout.js` checks, by totting up the printed rows against the printed total at 241 doses.
- **Nothing the fold search considered can be cheaper than what it chose.** The pool is sorted on difficulty before error, so a pool member with a lower score than the winner is a contradiction, not a near-miss. The comparison panel says so in as many words, and the sweep fails if that sentence ever renders. A closer fold losing is *normal*: being closer than the winner puts it inside the cap by construction, so it was always in the running and lost on difficulty alone.
